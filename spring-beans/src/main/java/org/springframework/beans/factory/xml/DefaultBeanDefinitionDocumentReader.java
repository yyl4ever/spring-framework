...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	public void registerBeanDefinitions(Document doc, XmlReaderContext readerContext) {
		...
		//获得 Document 的根元素：Element root = doc.getDocumentElement();
	 ...
	 */
	//解析 <import>导入元素 ， 从给定的￿导入路径加载 Bean 资源到 Spring IoC 容器中
	protected void importBeanDefinitionResource(Element ele) {
		//获取给定的导入元素的 location 属性
		String location = ele.getAttribute(RESOURCE_ATTRIBUTE);
		//如果导入元素 的 location 属性值为空，则没有导入任何资源，直接返回
	...
	protected void processAliasRegistration(Element ele) {
		...
		boolean valid = true;
		//<alias>别名元素 的 name 属性值为空
		if (!StringUtils.hasText(name)) {
			getReaderContext().error("Name must not be empty", ele);
			valid = false;
		}
		//<alias>别名元素 的 alias 属性值为空
		...
		if (valid) {
			try {
				//向容器的资源读入器注册别名
				//这里实际执行的是SimpleAliasRegistry中的registerAlias()方法
	...
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		//BeanDefinitionHolder 是对 BeanDefinition 的封装 ，即 Bean 定义 的封装类
		...
		if (bdHolder != null) {
			...
			try {
				...
				// 当调用 BeanDefinitionReaderUtils 向 Spring loC 容器注册解析的 BeanDefinition 时， 真正完成 注册功能的是 DefaultListableBeanFactory
			...
			//在完成向 Spring IoC容器注册解析得到的 Bean定义之后， 发送注册事件
